...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        ...
        hash = cryptoNight(convertedBlob, convertedBlob[0] >= 7 ? convertedBlob[0] - 7 : 0);
	log('info', logSystem, 'convertedBlob[0] %s', [convertedBlob[0]]);
	log('info', logSystem, 'convertedBlob %s', [convertedBlob]);
        shareType = 'valid';
    }
